...
Let us apply what we just learned and take a couple of minutes to play around 
with sequencing and sub-setting the vectors.
...
## End of Session exercises (part 4):

### Exercise 1:

Create a vector called `my_vector` with the values 1,2,3 and check the class of
...
```{r, eval = FALSE}
#Create the vectors "name", "age", and "major" and assign some values
name <-
age <- 
major <- 
...
Using the previously created `grade` or assign a new value, test your code!
...
### Bonus exercise 5: Writing your own function (hard!) {.tabset .tabset-fade .tabset-pills .unlisted}

<details>
<summary> Open Bonus exercise (Are you sure?)</summary>\ <details>
  <summary> Are you REALLY sure? </summary>
If you have made it this far and there is still time left, or you are curious to play around at home, here is a much more difficult exercise to get a glance at another very useful functionality in R: declaring your own functions!

To declare a new function, use `function()`. You can consult the documentation 
to get an overview of the command using `?"function"` or `help("function")` (quotation marks are mandatory in both cases). *We will also cover this together and in detail in session 5 of the course!*

While defining a function you can also define any number of arguments. By default, a function will return the *last evaluated expression*. Use `return()` to explicitly declare which object should be returned. You can use `return()` multiple times to return multiple objects, but you cannot use multiple arguments in a single call of `return()`.

Define a function and assess test scores to assign a grade similar to exercise 4. Then, apply your function to grade the test scores in the `scores` vector generated in the following chunk. **This is just to set up the exercise and you do not need to understand this code in detail!** The comments explain what each line of code achieves.

```{r generate test scores}
...
# Finally, we sample and store 20 exam scores, discarding values below 0 
# (everything wrong) and above 100 (everything correct).
scores <- sample(scores[scores >= 0 & scores <= 100], 20)

```


**Now it is your turn! Try to complete the code chunk below: Declare a function 
to grade test scores using the same conditions listed in exercise 4 and apply 
your function to grade the test scores in the `scores` vector.** 
In the *incomplete function below*, vector subsetting is used to target specific
ranges of test scores and assign grades, similar to what we have learned in 
section "Logical Operator" for room temperature. 
Alternatively, you can use the ifelse() function.

```{r define grading function, eval = F}
# define a function named grade_score()
grade_score <- 
  # complete list of arguments (arglist)
  # optional: declare default values for arguments using =
  function(score, threshold_A=90, threshold_B, threshold_C,  ) 
    # begin definition of function
    {
       # subset test scores to target only those scores which meet the condition         # of being equal to or larger than the threshold declared in "threshold_A"
       # and replace subset of scores with grade "A"
       score[score >= threshold_A] <- "A"
       # grade "B"
       score[score < threshold_A & score  ] <- "B"
       # grade "C"
...
If you have not done so already, now is the time to apply your newly declared 
function on the `scores` vector. To do so, simply call your function, e.g. `grade_students()` and supply the necessary arguments.

```{r apply grading function to test scores, eval = F}
# apply function to 'scores' vector
grade_score(score =  ,
            #threshold_A = 90, # not needed if default value is correct
            threshold_B =  ,
)

```

If this was too hard for now, do not despair! We will cover functions thoroughly
in session 4 of this course. Likely, you will find this task a lot easier by then.
You can look forward to your learning progress!
